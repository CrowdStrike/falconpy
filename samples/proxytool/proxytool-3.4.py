...
def main():
    ...
    if not args.proxy_disable:
        ...
        for store in registry_stores:
            ...
            response = falcon.batch_active_responder_command(batch_id=batch_id, base_command="reg set", command_string="reg set " + store + " DisableProxy -ValueType=REG_DWORD -Value=1")
            if response["status_code"] == 201:
                log("-- Issuing registry setting of DisableProxy to a non-zero value in " + store)
            else:
                log("Error, Response: " + response["status_code"] + " - " + response.text)
                exit()   
