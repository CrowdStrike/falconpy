...
# This tool grabs all spotlight vulnerabilities on an account given a filter derived from FQL: https://falconpy.io/Service-Collections/Spotlight-Vulnerabilities.html
...
def main():
    ...
    parser.add_argument('--client_id', type=str, required=True, help="The Client ID of your Falcno API Key")
    parser.add_argument('--output_file', type=str, required=False, help="The output file for the associated vulnerabilities", default="spotlight_vulnerabilities.txt")
    parser.add_argument('--filter', type=str, required=False, help="Filter for Vulnerabilities created via FQL: https://falconpy.io/Service-Collections/Spotlight-Vulnerabilities.html", default="status:!'closed'+last_seen_within:'3'+cve.exprt_rating:['CRITICAL']")
    parser.add_argument('--verbose', action="store_true", required=False, help="Give Verbose Information On Data Pull", default=False)
    args = parser.parse_args()

    api_secret = getpass("Enter your Falcon API Key: ")
    spotlight_data = get_all_vulnerabilites_from_account(args.client_id, api_secret, args.filter, args.verbose)
