...
_endpoint._real_time_response - Internal API endpoint constant library
    ...
    [
      {
        ...
        "Supported aggregation members:\n\n**`date_ranges`** If peforming a date range "
        "query specify the **`from`** and **`to`** date ranges.  These can be in common "
        "date formats like `2019-07-18` or `now`\n**`field`** Term you want to aggregate on.  "
        "If doing a `date_range` query, this is the date field you want to apply the date ranges "
        "to\n**`filter`** Optional filter criteria in the form of an FQL query. For more "
        "information about FQL queries, see our [FQL documentation in Falcon]"
  ...
  [
    ...
    [
      ...
      },
      {
        ...
        "Batch ID to execute the command on.  Received from `/real-time-response/combined/init-sessions/v1`.\n"
  ...
  [
    ...
    [
      ...
      },
      {
        ...
        "command type we are going to execute, for example: `ls` or `cd`.  Refer to the RTR documentation for "
        ...
        "List of a subset of hosts we want to run the command on.  If this list is supplied, only these "
        "hosts will receive the command.",
        "name": "body",
        "in": "body",
        "required": True
      }
  ...
  [
    ...
    [
      ...
      },
      {
        "description": "**`batch_id`** Batch ID to execute the command on.  "
        "Received from `/real-time-response/combined/init-sessions/v1`.\n**`hosts_to_remove`** "
        "Hosts to remove from the batch session.  Heartbeats will no longer happen on these hosts and "
        "the sessions will expire.",
        "name": "body",
        "in": "body",
        "required": True
      }
    ...
    [
      {
        ...
        "`reg unload`\n- `restart`\n- `rm`\n- `runscript`\n- `shutdown`\n- `unmap`\n- `update history`\n- "
        "`update install`\n- `update list`\n- `update query`\n- `xmemdump`\n- `zip`\n\nRequired values.  "
        "The rest of the fields are unused.\n**`base_command`** Active-Responder command type we are going "
        "to execute, for example: `get` or `cp`.  Refer to the RTR documentation for the full list of "
        "commands.\n**`command_string`** Full command string for the command. For example  `get some_file.txt`\n"
    ...
    [
      {
        ...
        "Full command string for the command. For example  `cd C:\\some_directory`\n**`session_id`** "
        "RTR session ID to run the command on",
        "name": "body",
        "in": "body",
        "required": True
      }
