    ...
    if returned_content_type.startswith("application/json"):  # Issue 708
        ...
        try:
            ...
            if "resources" in json_resp and not contain:
                ...
                # if revoke or auth:
                # Something's up with the revocation payload still.
    ...
    if api.method.upper() in _ALLOWED_METHODS:
        ...
        if api.perform:
            ...
            try:
                ...
                # !!! EXPERIMENTAL !!!
                # Think this needs to happen sooner honestly.
                if pythonic:
        ...
        for redact in redacted:
            ...
            if "body" in cleaned:
                if redact in cleaned["body"]:
                    cleaned["body"][redact] = "REDACTED"   # .        o
                if "resources" in cleaned["body"]:         # .           o
                    # Log results are limited to MAX_DEBUG_RECORDS   o O
